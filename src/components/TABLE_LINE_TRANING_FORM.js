...
const TABLE_LINE_TRANING_FORM = () => {
    ...
    const handlePageClick = async (data) => {
        ...
        if (Location === `Line`) {
            const responseData = await api.get('/Forms/TablesLineTraining', { params: { PerPage: 10, From: (data.selected) * 10 } })
            setData(responseData.data)
        }
        else if (Location === `Details`) {
            const responseData = await api.get('/Forms/TablesDetailsTraining', { params: { PerPage: 10, From: (data.selected) * 10 } })
            setData(responseData.data)
        }

        // const response = await api.get(`/Tasks/PaginatedTasks`, { params: { ID: decryptedData, From: (data.selected * 10) + 1, To: (data.selected + 1) * 10 } });
    ...
    const getLineTableData = async () => {
        const response = await api.get('/Forms/TablesLineTrainingID')
        setPagesNumber(Math.ceil((response.data.length) / 10))

        const responseData = await api.get('/Forms/TablesLineTraining', { params: { PerPage: 10, From: 0 } })
        setData(responseData.data)

    }

    const getDetailsTableData = async () => {
        const response = await api.get('/Forms/TablesDetailsTrainingID')
        setPagesNumber(Math.ceil((response.data.length) / 10))

        const responseData = await api.get('/Forms/TablesDetailsTraining', { params: { PerPage: 10, From: 0 } })
        setData(responseData.data)
    }
    return (
            ...
            <div className="flex flex-col">
                <div >
                    ...
                    <div className="p-1.5 w-full inline-block align-middle text-center">
                        <div className="  overflow-y-scroll  h-[60vh] mb-[30px] ">
                        ...
                        <ReactPaginate
                            ...
                            pageClassName={"flex justify-center items-center w-[40px] h-[40px] border-[3px] border-[#fd8503] rounded-[50%]"}
                            pageLinkClassName={"text-lg font-black text-[#470d67]"}
                            previousClassName={"px-2 py-2 bg-[#470d67] border-[2px] border-[#fd8503] rounded-lg "}
                            previousLinkClassName={"text-lg font-medium text-[#fd8503] "}
                            nextClassName={"px-2 py-2 bg-[#470d67] border-[2px] border-[#fd8503] rounded-lg "}
                            nextLinkClassName={"text-lg font-medium text-[#fd8503] "}
                            breakClassName={"flex justify-center items-center"}
                            breakLinkClassName={"text-lg font-black text-[#470d67]"}
                            activeClassName={"bg-[#fd8503] "}
                        />
                    </div>
                </div>
            </div>
